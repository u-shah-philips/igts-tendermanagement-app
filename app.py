...
# Hardcoded values
AGENT_ID = "" ##Update the agent_id
AGENT_ALIAS_ID = "" ## Update agent alias id
...
def format_references(references):
    ...
    for ref in references:
        for retrieved_ref in ref.get('retrievedReferences', []):
            ...
            # Replace s3:// with https:// for clickable URLs
            uri = uri.replace("s3://", "https://")
...
def main():
    ...
    if "citations_links" in st.session_state:
        ...
        for i, citation in enumerate(st.session_state["citations_links"], 1):
            st.write(f"[Citation {i}]({citation})")
